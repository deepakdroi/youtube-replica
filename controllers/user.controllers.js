...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (incomingRefreshToken !== user.refreshToken) {
      throw new ApiError(401, "Refreh token has expired or used");
    }
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "Avatar image uldated successfully"));
});
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  //delete acover image from cloudinary
  const user = await User.findByIdAndUpdate(
    req.user?._id,
    {
      $set: {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "Cover image uldated successfully"));
});
  ...
  const channel = await User.aggregate([
    ...
    },
    {
      $addFields: {
        ...
        isSubscribed: {
          $cond: {
            of: { $in: [req.user?._id, "$subscribers.subscriber"] },
            then: true,
            else: false,
          },
        },
